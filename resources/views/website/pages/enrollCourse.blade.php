...
<form class="auth-form login-form w-100" method="post" action="{{ url('addenroll/'.Session::get('userid').'/'.$sid) }} "
    enctype="multipart/form-data">
    {{ csrf_field() }}
    
    <div class="myaccordion">
        <h3 class="mt-3 auth-heading text-center mb-4">On <span class="text-success">{{$ssession}}</span> Session's Offered Courses List</h2>
...
<script>
    $(document).ready(function () {
        ...
        $('.section-title').one('click', function () {
            ...
            $s = '';
            $.ajax({
                url: "http://127.0.0.1:8000/api/offered-subj",
                type: 'GET',
                data:{
                    semester:$id,
                    session_id:{{$sid}},
